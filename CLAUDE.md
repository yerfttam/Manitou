...
NEW/                  ← source of truth, this is what gets deployed
  ...
  accommodations.html ← Property listings (rendered from listings.json via JS)
...
```bash
python3 -m http.server 3456 --directory NEW
```
...
- `index.html` ✅ — Home (hero section done; remaining sections in progress)
- `accommodations.html` ✅ — Property cards rendered from `listings.json` by JS, grouped by category
...
- `specials.html` — Not yet built
- `reservations.html` — Not yet built
...
**Note:** Resources was replaced with Policies. Contact page not yet built.
